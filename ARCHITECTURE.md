...
每次 workflow run 会为已装配的 reranker 创建独立 `RerankerRun`，把并行任务与修复轮次纳入同一调用上限；查询/候选仅通过摘要参与缓存键，`run.retrieval_usage` 只输出调用、缓存命中、跳过、失败和延迟统计。
...
- 语义 verdict 区分支持、部分支持、冲突和证据不足；冲突/不足不会被当作“可接受的不确定”，而会生成关系化支持边并触发修复或人工复核。
...
- 质量指标只消费 ID、排序和评估状态；`run.quality` 不包含查询、病历、证据原文或模型回答。
- embedding provider 使用有界缓存和批量 miss 合并，`run.retrieval_usage.embedding` 仅记录计数、命中、失败和延迟，不记录文本或向量。
